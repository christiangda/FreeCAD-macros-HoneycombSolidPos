...
class HoneycombSolidPos:
	...
	def execute(self, fp):
		...
		internal_box = Part.makeBox(length,width,height)
		external_box = Part.makeBox(2*length,2*width,height)

		external_box_origin_x_y = Base.Vector((length/2)*-1, (width/2)*-1,0)
		external_box.translate(external_box_origin_x_y)
		box_frame = external_box.cut(internal_box)
		...
		n_cols = math.ceil( (length / (radius + tickness) / 2) - 1)
		...
		shape = extruded_polygon.cut(box_frame) # comment it to see the array of solids
...
class ViewProviderHoneycombSolidPos:
	...
	def getIcon(self):
		...
		return """
			/* XPM */
			static const char * ViewProviderHoneycombSolidPos_xpm[] = {
			"16 16 54 1",
			" 	c None",
			".	c #181818",
			"+	c #5F5F5F",
...
def makeHoneycombSolidPos(version):
	doc = FreeCAD.activeDocument()

	if doc is None:
		doc = FreeCAD.newDocument()

	obj = doc.addObject("Part::FeaturePython", "HoneycombSolidPos")
	HoneycombSolidPos(obj,version)
	ViewProviderHoneycombSolidPos(obj.ViewObject)
	doc.recompute()
